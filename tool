...
-- 5. Invisible (座標偏移與半透明) 控制系統
local u7 = {}
local function updateTransparentParts()
    u7 = {}
    local chr = player.Character
    if chr then
...
local function toggleInvisibleVisual()
    for _, part in ipairs(u7) do
...
local function applyInvisibleSystem()
    -- 清除舊全域監聽
    if _G.a then
        for _, conn in ipairs(_G.a) do
    ...
    -- 鍵盤 G 鍵按鈕觸發
    v23[1] = mouse.KeyDown:Connect(function(key)
        if key == "g" then
            InvisibleActive = not InvisibleActive
            toggleInvisibleVisual()
        end
    end)

    -- 座標偏移 Loop
    v23[2] = RunService.Heartbeat:Connect(function()
        if InvisibleActive and player.Character then
            local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
            local humanoidRootPart = player.Character:FindFirstChild("HumanoidRootPart")
            if humanoid and humanoidRootPart then
                local cFrame = humanoidRootPart.CFrame
                local cameraOffset = humanoid.CameraOffset
                local v29 = cFrame * CFrame.new(0, -200000, 0)
                local position = v29:ToObjectSpace(CFrame.new(cFrame.Position)).Position

                humanoidRootPart.CFrame = v29
...
Frame.Size = UDim2.new(0, 210, 0, 314)
Frame.Position = UDim2.new(0.5, -105, 0.5, -157)
...
-- 【2】Invisible 隱身/座標無敵開關 (放在 God Mode 正下方)
local InvisibleToggleFrame = Instance.new("Frame", Content)
InvisibleToggleFrame.Size = UDim2.new(1, 0, 0, 28); InvisibleToggleFrame.Position = UDim2.new(0, 0, 0, 40)
InvisibleToggleFrame.BackgroundTransparency = 1

local InvisibleToggleBtn = Instance.new("TextButton", InvisibleToggleFrame)
InvisibleToggleBtn.Size = UDim2.new(1, 0, 1, 0); InvisibleToggleBtn.Text = "隱身/座標無敵 (Invisible): 關閉"; InvisibleToggleBtn.Font = Enum.Font.GothamBold; InvisibleToggleBtn.TextSize = 11
InvisibleToggleBtn.BackgroundColor3 = Themes.Dark.Button; InvisibleToggleBtn.TextColor3 = Themes.Dark.Text; InvisibleToggleBtn.BorderSizePixel = 0
Instance.new("UICorner", InvisibleToggleBtn).CornerRadius = UDim.new(0, 6)

-- 【3】速度調整組合框
local SpeedFrame = Instance.new("Frame", Content)
SpeedFrame.Size = UDim2.new(1, 0, 0, 28); SpeedFrame.Position = UDim2.new(0, 0, 0, 74)
...
JumpFrame.Size = UDim2.new(1, 0, 0, 28); JumpFrame.Position = UDim2.new(0, 0, 0, 108)
...
FlyFrame.Size = UDim2.new(1, 0, 0, 28); FlyFrame.Position = UDim2.new(0, 0, 0, 142)
...
FlyToggleFrame.Size = UDim2.new(1, 0, 0, 28); FlyToggleFrame.Position = UDim2.new(0, 0, 0, 176)
...
HeightFrame.Size = UDim2.new(1, 0, 0, 28); HeightFrame.Position = UDim2.new(0, 0, 0, 210)
...
table.insert(buttonReferences, InvisibleToggleBtn)
...
InvisibleToggleBtn.MouseButton1Click:Connect(function()
    InvisibleActive = not InvisibleActive
    InvisibleToggleBtn.Text = "隱身/座標無敵 (Invisible): " .. (InvisibleActive and "開啟" or "關閉")
    toggleInvisibleVisual()
end)
...
SettingsBtn.MouseButton1Click:Connect(function()
    ...
    if isMinimized then
        isMinimized = false
        TweenService:Create(Frame, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 210, 0, 314)}):Play()
...
TweenService:Create(Frame, TweenInfo.new(0.1, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
    Size = UDim2.new(0, 210, 0, 314),
    Position = UDim2.new(0.5, -105, 0.5, -157)
}):Play()
...
game:GetService("StarterGui"):SetCore("SendNotification", { 
    ...
    Text = "選單已新增獨立 Invisible (隱身/座標無敵) 按鈕 (亦支援快捷鍵 G)，by: Baiyan";
    Duration = 4
})
